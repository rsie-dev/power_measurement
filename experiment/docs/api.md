...
```python
with_stable_temp_head_delay(min_delay: int, max_delay: int, threshold: float = 1) -> Self
```
Adds a temperature stabilized delay before execution starts.

```python
with_stable_temp_tail_delay(min_delay: int, max_delay: int, threshold: float = 1) -> Self
```
