...
class TimedCommand(Command):
    ...
    def execute(self, nr: int, connection: Connection, resources_path):
        ...
        self._logger.info("Execute[%02d,%s]: %s", nr, exec_info, self._timed_command.command)
        with ((connection.cd(work_dir))):
            connection.run(command, hide=True)
            time_info = BytesIO()
            connection.get(remote=timing_output, local=time_info)
            timing_entry = self._extract_timing_entry(nr, time_info)
            timings = (
    ...
    def _extract_timing_entry(self, nr: int, time_file: BytesIO) -> TimingEntry:
        ...
        return TimingEntry(entry_nr=nr,
                           ...
                           user=entries["user"],
                           sys=entries["sys"],
                           command=self._timed_command.command)
