...
from experiment.run.log import Logger, LogDispatcher
    ...
    def _execute_commands(self, connection: Connection):
        with StartupMonitor(self._host.host_name, self._metrics_dispatcher) as startup_monitor:
            ...
            if not startup_event.wait(self._metrics_client_timeout):
                raise TimeoutError(f"System metrics thread did not start within " f"{self._metrics_client_timeout} seconds")
