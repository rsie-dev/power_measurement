from pyinfra.operations import apt
from pyinfra import host

from filesystem import remount_rw
from base import base, base_filesystem
from develop import develop
from network import base_network, router
from ntp import ntp_server, ntp_client
from telegraf import telegraf
from compressors import compressors, stressors
from system import switch_to_read_only, add_home_partition, set_apt_proxy
from system import unify_memory_size, unify_fan_speed, unify_tmp_filesystem
from ssh import install_ssh_keys, prohibit_root_login
from data import setup_data_folder
from users import add_user, add_users, allow_user_telegraf_control, allow_user_cache_control, allow_user_timer_control

remount_rw(
    name=f"Ensure / is mounted RW",
    path="/",
    _sudo=True,
)

use_proxy = host.data.get("use_proxy", False)
set_apt_proxy(use_proxy)

base()
base_network()
stressors()
compressors()
prohibit_root_login()

if "controller" in host.groups:
    develop()
    ntp_server()
    router()
    base_filesystem()
    add_home_partition()
    sync_data = host.data.get("sync_data", False)
    setup_data_folder(sync_data)
    add_users()


if "dut" in host.groups:
    if host.data.get("install_cpupower", True):
        apt.packages(
            name="Install CPU power util",
            packages=["linux-cpupower"],
            no_recommends=True,
            _sudo=True,
        )

    ntp_client()
    telegraf()
    test_user = "ctest"
    add_user(
        name="Create user: %s" % test_user,
        user=test_user,
        _sudo=True,
    )
    allow_user_telegraf_control(test_user)
    allow_user_cache_control(test_user)
    allow_user_timer_control(test_user)
    install_ssh_keys(test_user)
    unify_memory_size()
    unify_tmp_filesystem()
    unify_fan_speed()

switch_to_read_only()
if host.data.get("use_proxy", False):
    set_apt_proxy(False)
